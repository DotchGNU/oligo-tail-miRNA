setwd("~/Desktop/R_scripts/F2L3 AGO2IP comparison/AGO1_mutants_TCGA/")
setwd("~/Desktop/P6 Tiejuan/02_F2L3 AGO2IP comparison/AGO1_mutants_TCGA/")
library(ggplot2)

manifest_data <- read.table(file="manifest_all_AGO1_mutants_TCGA3.txt", header = TRUE, fill = TRUE, sep =  "\t")
...
isomir_sequences <- read.table(file="AGO1_mutants_TCGA_run2_5.isomir.sequence_info.tsv", header = TRUE, sep =  "\t")
...
for (i in 1:n) {
  ...
  mut_P295L <- mir21[which(mir21$AGO1_mut==mutation),]
  if(nrow(mut_P295L)>0){
    avg_expression$P295L[i] <- mir21[which(mir21$AGO1_mut==mutation),16]
  }
  mir21 <- merge(mir21,mut_to_domain, by="AGO1_mut")
...
lines(avg_expression$synonimous_iso, avg_expression$missense_iso,col="red")
abline(a=1, b=1)
...
mutation2 <- gsub("/","_",mutation)
name_file <- paste0("AGO1_missense_synonimous_",mutation2,"_miRNA2.txt")
name_file <- "AGO1_missense_synonimous_domains_miRNA2.txt"
